...
SCharacterController* SFirstPersonController::getCharacterController() {
	return (SCharacterController*)fields.__________13;
}
...
float STime::getTime() {
	return app::Time_1_get_time(nullptr);
}
...
float SPlayerStamina::getDeltaTime() {
	return app::Time_1_get_deltaTime(nullptr);
}

bool SPlayerStamina::isWalking() {
	return fields.__________1; //[s
}
...
float SPlayerStamina::getStamina() {
	return fields.__________11;//u0D21u0D1Du0D3Bu0D1Eu0D22u0D31u0D2Eu0D36u0D38;
}
...
SKeyType SKey::getType() {
	if (fields.info->fields.keyType == app::___Enum_15::Main)
...
SObject_Array* SObject_Array::of() {
	return (SObject_Array*)app::Array_Empty_15(*app::Array_Empty_16__MethodInfo);
}
...
bool SGhostTraits::get12() {
	//return u0D3Bu0D22u0D34u0D2Au0D39u0D3Au0D2Fu0D21u0D2B;
}
...
SAnimator* SGhostAI::getAnimator() {
	...
	return (SAnimator*)ghostModel->fields.__________2;
}
...
void SOuijaBoard::sendResponse(SString* response) {
	//app::OuijaBoard_u0D23u0D1Cu0D2Du0D31u0D30u0D38u0D21u0D24u0D1E(this, response, nullptr);
}
